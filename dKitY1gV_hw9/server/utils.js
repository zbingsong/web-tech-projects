...
export function extractArtistDetail(artistList) {
  ...
  // followers
  const followerNum = artistList[0].followers.total;
  if (followerNum < 1000) {
    extractedInfo.followers = followerNum.toString() + ' Followers';
  } else if (followerNum < 1000000) {
    extractedInfo.followers = Math.round(followerNum / 1000) + 'K Followers';
  } else if (followerNum < 1000000000) {
    extractedInfo.followers = Math.round(followerNum / 1000000) + 'M Followers';
  } else {
    extractedInfo.followers = Math.round(followerNum / 1000000000) + 'B Followers';
  }
  return extractedInfo;
}
...
export function extractVenueDetail(venue) {
  const extractedInfo = {
    ...
    open_hours: venue.boxOfficeInfo?.openHoursDetail ?? '',
    gen_rule: venue.generalInfo?.generalRule ?? '',
    child_rule: venue.generalInfo?.childRule ?? '',
    location: {
      lat: 0,
      lng: 0,
    },
  ...
  extractedInfo.city_state = venueAddr.length > 0 ? venueAddr.join(', ') : '';
  // location
